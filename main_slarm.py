...
def get_args_parser():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="skip startup and periodic training videos; evaluation is unaffected",
    )
    ...
    parser.add_argument(
        "--save_best_ckpt",
        action="store_true",
        help="save an additional best.pth checkpoint using validation PSNR",
    )
...
def maybe_save_best_checkpoint(
    ...
    """Persist the best validation checkpoint without affecting rolling checkpoints."""
    if not args.save_best_ckpt or eval_result is None or "psnr" not in eval_result:
        return best_eval_psnr

    eval_psnr = float(eval_result["psnr"])
    if not math.isfinite(eval_psnr) or eval_psnr <= best_eval_psnr:
        return best_eval_psnr
    ...
    checkpoint = {
        ...
        "args": args,
        "total_elapsed_time": time.time() - start_time + args.total_elapsed_time,
    ...
    with open(os.path.join(args.ckpt_dir, "best_psnr.txt"), "w") as f:
        f.write(f"{eval_psnr:.10f}\n")
    logger.info(
        f"Updated best checkpoint to {best_path} "
        f"(PSNR={eval_psnr:.4f}, iteration={data_iter_step})"
    )
    return eval_psnr
...
def main(args):
    ...
    best_metric_path = os.path.join(args.ckpt_dir, "best_psnr.txt")
    if args.save_best_ckpt and os.path.exists(best_metric_path):
        try:
            with open(best_metric_path, "r") as f:
                best_eval_psnr = float(f.read().strip())
        except (OSError, ValueError):
            logger.warning(f"Could not read best checkpoint metric from {best_metric_path}")
    ...
    if dataset_val is not None and valid_slice_id >= len(dataset_val):
        valid_slice_id = 0
    if args.visualization_only or not args.disable_visualization:
    ...
    for data_dict in metric_logger.log_every(
        ...
        if (
            ...
            and (data_iter_step + 1) % args.vis_every_n_iters == 0
        ):
            if distributed.is_main_process():
                for _ in range(args.num_vis_samples):
                    vis_slice_id, valid_slice_id = visualize(
        ...
        if (data_iter_step + 1) % args.eval_every_n_iters == 0 and (
            ...
            best_eval_psnr = maybe_save_best_checkpoint(
                args,
                model_without_ddp,
                optimizer,
                loss_scaler,
