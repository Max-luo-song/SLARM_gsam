...
def make_sequence_video(
    ...
    """Render consecutive clips and stitch them into one full-scene video.

    Each model invocation still uses the dataset's native clip length (currently
    16 frames for interpolation-only training). For a short final clip, the
    window is shifted backwards and its already-rendered prefix is discarded so
    every source timestep appears exactly once in the output video.
    """
    scene_id = scene_id % len(dataset)

    # Validation can be a concatenation of multiple datasets. Resolve the
    # selected global index to the underlying dataset and its local scene index.
    while hasattr(dataset, "datasets") and hasattr(dataset, "cumulative_sizes"):
        dataset_idx = bisect_right(dataset.cumulative_sizes, scene_id)
        previous_size = 0 if dataset_idx == 0 else dataset.cumulative_sizes[dataset_idx - 1]
        scene_id -= previous_size
        dataset = dataset.datasets[dataset_idx]

    scene_json = dataset.annotations[scene_id % len(dataset.annotations)]
    num_timesteps = scene_json["num_timesteps"]
    fps = scene_json["fps"]
    interval = dataset.get_interval(fps)
    video_frames = []

    for requested_start in range(0, num_timesteps, interval):
        segment_start = requested_start
        if segment_start + interval > num_timesteps:
            segment_start = max(0, num_timesteps - interval)

        data_dict = dataset.get_segment(
            index=scene_id,
            context_frame_idx=segment_start,
            return_all=True,
        )
        data_dict = to_batch_tensor(data_dict)
        segment_frames = make_video(
            dataset=None,
            model=model,
            device=device,
            data_dict=data_dict,
        ...
        # The last shifted window overlaps the preceding window. Keep only the
        # suffix beginning at the next not-yet-emitted source frame.
        relative_start = requested_start - segment_start
        video_frames.extend(segment_frames[relative_start:])
