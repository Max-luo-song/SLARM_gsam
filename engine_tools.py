...
from src.visualization.video_maker import make_sequence_video, make_video
...
def visualize(args, model, dset_train, step, train_vis_id, device,
    ...
    for vis_id, dataset in zip([train_vis_id, val_vis_id], [dset_train, dset_val]):
        ...
        if split == "val":
            # Render consecutive native-length clips and stitch them in temporal
            # order, so validation videos show the dynamics of the whole scene.
            video = make_sequence_video(
                dataset,
                model,
                device,
